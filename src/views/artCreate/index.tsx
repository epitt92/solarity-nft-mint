...
export const ArtCreateView = () => {
  ...
  // ctx.drawImage(PSS, 0,0, 100, 100)
  const getSocialInfo = async function(){
  }
  ...
  useEffect(() => {
    let imgData = document.getElementById("passport")
    let canvas =  document.getElementById("canva");
    if(step == 2 && canvas !== null){
      let ctx = canvas.getContext("2d");
      ctx.drawImage(imgData, 0,0, 432, 300)
      var img = new Image;
      img.onload = function() {
          ctx.drawImage(img, 24,75, 140, 164);
          ctx.font = "18px Comic Sans MS";
          ctx.fillStyle = "black";
          ctx.fillText(attributes.name, 280, 90);
          axios.get(`https://solarity-server.herokuapp.com/api/users/tmeta?includeDao=true`).then(response => {
            ...
            if( 'twitterConnected' in resp['user'] ){
              ...
              console.log("attributes-log", typeof nftattributes)
              ctx.fillText(resp['user']['twitterUsername'], 280, 110);
            }
            // if( resp['user']['twitterConnected'] === false ){
            //   ctx.fillText(resp['user']['twitterUsername'], 280, 110);
            // }
            let startY = 130
            resp['user']['daos'].forEach(element => {
              ...
              nftattributes.push(nAttribute);
              ctx.fillText(element['name'], 280, startY);
              startY += 20;
            });
            // setAttributes({
            //   ...attributes,
  ...
  return (
    <>
      <img style={{"display":"none"}} src="/img/passport.png" id="passport" alt="passport" />
      <canvas id="canva" width="432px" height="300px" style={{'display':'none', "backgroundColor": "red"}}></canvas>
  ...
  return (
    <>
      <Row className="call-to-action">
        ...
        <p>
          Which kind of media are you gonna use for passport?{' '}
          {/* <a
...
const UploadStep = (props: {
  ...
  return (
    <>
      <Row className="call-to-action">
        <h2>Now, let&apos;s upload your passport image</h2>
      </Row>
      <Row className="content-action">
        <h3>Upload a passport image (PNG, JPG, GIF, SVG)</h3>
        <Dragger
    ...
    >
      ...
      <Card>
        <Steps direction="vertical" current={props.step}>
          ...
          <Step
            className={'white-description'}
            title="Uploading to NFT.Storage"
